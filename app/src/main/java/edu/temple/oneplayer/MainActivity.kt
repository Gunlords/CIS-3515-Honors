...
class MainActivity : AppCompatActivity(), BookControlFragment.BookControlInterface  {
    ...
    private fun saveBookProgress(book: Book, position: Int) {
        val prefs = getSharedPreferences("book_progress", MODE_PRIVATE)
        prefs.edit()
            .putInt("${book.book_id}_position", position)
            .putLong("${book.book_id}_timestamp", System.currentTimeMillis())
            .apply()
    }

    private fun getBookProgress(book: Book): Pair<Int, Long>? {
        val prefs = getSharedPreferences("book_progress", MODE_PRIVATE)
        if (!prefs.contains("${book.book_id}_position")) return null
        val position = prefs.getInt("${book.book_id}_position", 0)
        val timestamp = prefs.getLong("${book.book_id}_timestamp", 0)
        return Pair(position, timestamp)
    }
            ...
            JsonArrayRequest(searchURL + searchTerm,
                {
                    ...
                    saveLastSearch(it.toString())
                },
                { Toast.makeText(this, it.networkResponse.toString(), Toast.LENGTH_SHORT).show() })
        )
    }
    private fun saveLastSearch(json: String) {
        val file = File(filesDir, "last_search.json")
        file.writeText(json)
    }
    ...
    override fun playBook() {
        bookViewModel.getSelectedBook()?.value?.apply {
            mediaControllerBinder?.run {
                ...
                val savedProgress = getBookProgress(this@apply)?.first ?: 0
                play(this@apply, savedProgress)

                Toast.makeText(
                    this@MainActivity,
                    "Resuming Book ID: $book_id from $savedProgress ms",
                    Toast.LENGTH_SHORT
                ).show()

                // Start downloading in background (even if playing)
                downloadBookFile(this@apply)
            }
        }
    }
    ...
    override fun pauseBook() {
        mediaControllerBinder?.run {
            ...
            bookViewModel.getPlayingBook()?.value?.let { book ->
                saveBookProgress(book, lastKnownProgress)
            }
    ...
    private fun loadLastSearchIfExists() {
        val file = File(filesDir, "last_search.json")
        if (file.exists()) {
            try {
                val json = file.readText()
                val jsonArray = org.json.JSONArray(json)
                bookViewModel.updateBooks(jsonArray)
                Log.d("SearchRestore", "Loaded last search results")
            } catch (e: Exception) {
                Log.e("SearchRestore", "Failed to load saved search: ${e.message}")
            }
        }
    }
    ...
    private fun getBookFile(book: Book): File {
        return File(filesDir, "book_${book.book_id}.mp3")
    }

    private fun downloadBookFile(book: Book) {
        val file = getBookFile(book)
        if (!file.exists()) {
            Thread {
                try {
                    val url = java.net.URL("https://kamorris.com/lab/oneplayer/downloadbook.php?id=${book.book_id}")
                    url.openStream().use { input ->
                        file.outputStream().use { output ->
                            input.copyTo(output)
                        }
                    }
                    Log.d("Download", "Downloaded book to ${file.absolutePath}")

                    // After downloading, link the book to the local file
                    book.bookFile = file
            ...
            // If already exists, link immediately
            book.bookFile = file
        }
    }
